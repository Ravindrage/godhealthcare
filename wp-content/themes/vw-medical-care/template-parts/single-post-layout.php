...
<article id="post-<?php the_ID(); ?>" <?php post_class('inner-service'); ?>>
    <div class="single-post">
        ...
        <?php if( get_theme_mod( 'vw_medical_care_toggle_postdate',true) != '' || get_theme_mod( 'vw_medical_care_toggle_author',true) != '' || get_theme_mod( 'vw_medical_care_toggle_comments',true) != '') { ?>
            <div class="post-info">
                <?php if(get_theme_mod('vw_medical_care_toggle_postdate',true)==1){ ?>
                    <i class="fas fa-calendar-alt"></i><span class="entry-date"><a href="<?php echo esc_url( get_day_link( $vw_medical_care_archive_year, $vw_medical_care_archive_month, $vw_medical_care_archive_day)); ?>"><?php echo esc_html( get_the_date() ); ?><span class="screen-reader-text"><?php echo esc_html( get_the_date() ); ?></span></a></span><span>|</span>
                <?php } ?>
        ...
        <div class="entry-content">
            <?php the_content(); ?>
            <?php if(get_theme_mod('vw_medical_care_toggle_tags',true)==1){ ?>
        ...
        <?php
            ...
            if ( is_singular( 'attachment' ) ) {
                ...
                the_post_navigation( array(
                    'next_text' => '<span class="meta-nav" aria-hidden="true">' .esc_html(get_theme_mod('vw_medical_care_single_blog_next_navigation_text','NEXT')) . '</span> ' .
                        '<span class="screen-reader-text">' . __( 'Next post:', 'vw-medical-care' ) . '</span> ' .
                        '<span class="post-title">%title</span>',
                    'prev_text' => '<span class="meta-nav" aria-hidden="true">' .esc_html(get_theme_mod('vw_medical_care_single_blog_prev_navigation_text','PREVIOUS')) . '</span> ' .
